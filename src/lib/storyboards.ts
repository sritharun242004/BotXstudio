...
export const IMAGE_GENERATION_MODELS = [
  {
    ...
    mode: "Flash Mode",
    tag: "Free Included",
    desc: "Quick high-quality ecommerce shots · ideal for everyday catalogues · included in free credits",
    colorClass: "modelCard--flash",
    provider: "Google Gemini 2.5 Flash",
    tokenBadge: "🌿 Flash — fast ecommerce generation · 5 credits/img",
    freeEligible: true,
  },
  {
    id: "hybrid-editorial",
    label: "Plus Model",
    mode: "Plus Workflow",
    tag: "Credits Required",
    desc: "Gemini Flash + FLUX Kontext angles · multi-view lookbook · best angle consistency",
    colorClass: "modelCard--hybrid",
    provider: "Google × fal.ai FLUX",
    tokenBadge: "⚡ Plus — Gemini Flash + FLUX Kontext Multi · 15 credits/set",
    freeEligible: false,
  },
  {
    id: "gpt-image-2",
    ...
    mode: "Pro Editorial",
    tag: "Credits Required",
    desc: "OpenAI GPT-4o vision · consistent angles · cinematic editorial quality · medium or high resolution",
    colorClass: "modelCard--gpt",
    provider: "OpenAI GPT Image 2",
    tokenBadge: "🏆 Pro — openai/gpt-image-2/edit · 6–25 credits/img",
    freeEligible: false,
  },
  {
    ...
    mode: "ProMax Mode",
    tag: "Free Included",
    desc: "Google's most powerful image model · ultra-detailed outputs · full custom controls · included in free credits",
    colorClass: "modelCard--pro",
    provider: "Google Gemini 3 Pro",
    tokenBadge: "🔥 ProMax — maximum Gemini quality · 20 credits/img",
    freeEligible: true,
  },
] as const;
...
export type StoryboardConfig = {
  ...
  generationTier: import("./generationTiers").GenerationTierId;
  ...
  gptImageSize: "1024x768" | "1024x1024";
  gptImageQuality: "medium" | "high";
};
...
export function createDefaultStoryboardConfig(): StoryboardConfig {
  return {
    ...
    generationTier: "standard_studio",
    occasionPreset: "",
    occasionDetails: "",
    accessories: "",
    bottomWearPreset: "",
...
function normalizeConfig(value: unknown): StoryboardConfig {
  ...
  const rawTier = asString(raw.generationTier) as import("./generationTiers").GenerationTierId | null;
  const generationTier: import("./generationTiers").GenerationTierId =
    rawTier && ["fast_draft", "standard_studio", "premium_editorial"].includes(rawTier)
      ? rawTier
      : "standard_studio";
  ...
  return {
    ...
    gptImageSize: (["1024x768", "1024x1024"].includes(asString(raw.gptImageSize) ?? "") ? asString(raw.gptImageSize) : base.gptImageSize) as "1024x768" | "1024x1024",
    gptImageQuality: (asString(raw.gptImageQuality) === "high" ? "high" : "medium") as "medium" | "high",
  };
}
