...
export function buildMultiAnglePrompt(opts: {
  ...
  if (opts.angle === "side") {
    ...
    lines.push(
      ...
      "Angle: IMAGE 1 (garment reference) and IMAGE 2 (main result) are both straight front-facing. This detail shot MUST use a DIFFERENT angle — a slight 3/4 turn or slight side angle — to provide visual variety and reveal garment depth and fabric texture.",
  ...
  if (opts.angle === "detail") {
    lines.push(
      "FINAL CHECK (non-negotiable):",
      "This is a DETAIL SHOT — do NOT output a full-body image. Use a different angle than IMAGE 1 and IMAGE 2 (slight 3/4 or side turn).",
      "Crop tightly to the garment focus region. Garment fills at least 80% of frame.",
      MODEL_AGE_RULE,
...
// ────────────────────────────────────────────────────────────────────────────
// Flash master prompt builder (composite / front view)
// Architecture: isolated reference images, one attribute per image slot.
// IMAGE 1 = garment · IMAGE 2 = model · IMAGE 3 = pose · IMAGE 4 = background
// ────────────────────────────────────────────────────────────────────────────
function buildFlashDirectCompositePrompt(opts: {
  ...
  // Flash image order: garment(1) → pose(2) → background(3) → model_LAST(4)
  // Model placed LAST for recency bias — reduces IMAGE_OTHER content rejection.
  let _idx = 2;
  const I_POSE  = opts.hasPoseReference       ? _idx++ : null;  // IMAGE 2
  const I_BG    = opts.hasBackgroundReference ? _idx++ : null;  // IMAGE 3
  const I_MODEL = opts.hasModelReference      ? _idx++ : null;  // IMAGE 4 (LAST)
  ...
  // ── MODEL CONFIG values ───────────────────────────────────────────────────
  const gender     = (opts.config.modelGender    || "Female").trim();
  const ageRange   = opts.config.modelAgeRange?.trim() || "18–23";
  const ethnicity  = opts.config.modelEthnicity?.trim() || "";
  const occasion   = opts.config.occasion?.trim()        || "casual";
  const accessories = opts.config.accessories?.trim()   || "minimal, matching the outfit";
  const footwear   = opts.config.footwear?.trim()        || "appropriate for occasion";
  const bottomWear = opts.config.bottomWear?.trim()      || "matching bottom wear";
  const stylingNotes = [
    !opts.hasModelReference && opts.config.modelStylingNotes?.trim(),
    opts.config.styleKeywords?.trim() && `style: ${opts.config.styleKeywords}`,
  ].filter(Boolean).join("; ") || "natural glam, commercial ecommerce";
  const poseName   = opts.hasPoseReference ? "from POSE REFERENCE (IMAGE 3)" : (opts.config.modelPose?.trim() || "natural standing, weight shift, relaxed hands");
  const bgTheme    = opts.config.backgroundTheme || opts.config.occasion || "clean lifestyle setting";

  // ── REFERENCE ISOLATION RULES ────────────────────────────────────────────
  lines.push(
    "You are generating a photorealistic ecommerce fashion image.",
    "",
    "========================================",
    "REFERENCE ISOLATION RULES",
    "========================================",
    "",
    "Each reference image controls ONLY its assigned attribute.",
    "",
    "IMAGE 1 = GARMENT REFERENCE",
    "Use ONLY for: garment design, garment color, garment texture, garment print, garment structure, garment stitching, garment silhouette.",
    "Do NOT copy from IMAGE 1: model identity, pose, background, lighting.",
    "",
  );

  if (I_MODEL !== null) {
    lines.push(
      `IMAGE ${I_MODEL} = MODEL REFERENCE`,
      "Use ONLY for: face identity, skin tone, hair, body type, age appearance, gender appearance.",
      `Do NOT copy from IMAGE ${I_MODEL}: garment, pose, background, accessories.`,
      "",
    );
  }
  ...
  if (I_POSE !== null) {
    lines.push(
      `IMAGE ${I_POSE} = POSE REFERENCE`,
      "Use ONLY for: body pose, skeleton structure, arm angles, leg placement, stance, body orientation.",
      `Do NOT copy from IMAGE ${I_POSE}: face, garment, hairstyle, background, lighting, accessories.`,
      "",
    );
  }
  ...
  if (I_BG !== null) {
    lines.push(
      `IMAGE ${I_BG} = BACKGROUND REFERENCE`,
      "Use ONLY for: environment, scene layout, atmosphere, background colors, location style.",
      `Do NOT copy from IMAGE ${I_BG}: people, pose, clothing, accessories.`,
      "",
    );
  }
  ...
  lines.push(
    ...
    "",
    "Output must show:",
    "- same model identity across all views",
    "- same garment",
    "- same styling",
  ...
  lines.push(
    ...
    "========================================",
    "",
    "Generate a clean ecommerce fashion image.",
    "",
    "Requirements:",
    "- full-body portrait",
    ...
    I_MODEL !== null ? `- identity from IMAGE ${I_MODEL}` : `- identity from MODEL CONFIG below`,
    I_POSE  !== null ? `- pose from IMAGE ${I_POSE}` : `- pose from MODEL CONFIG below`,
    I_BG    !== null ? `- background from IMAGE ${I_BG}` : `- background from MODEL CONFIG below`,
    "",
    "Garment must remain accurate:",
    "- exact color",
    "- exact print",
    "- exact silhouette",
    "- exact sleeve shape",
  ...
  if (I_MODEL !== null) {
    lines.push(
      "Model must remain consistent:",
      `- same face as IMAGE ${I_MODEL}`,
      `- same hair as IMAGE ${I_MODEL}`,
      `- same body type as IMAGE ${I_MODEL}`,
      "",
    );
  }
  ...
  if (I_POSE !== null) {
    lines.push(
      "Pose must follow:",
      `- same stance as IMAGE ${I_POSE}`,
      `- same limb positioning as IMAGE ${I_POSE}`,
      `- same orientation as IMAGE ${I_POSE}`,
      `- copy skeleton/joint angles ONLY from IMAGE ${I_POSE}`,
      "",
    );
  }
  ...
  lines.push(
    ...
    "========================================",
    "",
    `Garment Type: ${opts.config.styleKeywords || "(from IMAGE 1)"}`,
    ...
    `Occasion: ${occasion}`,
    I_BG === null ? `Background Theme: ${bgTheme}` : "",
    I_POSE === null ? `Pose: ${poseName}` : "",
    I_MODEL === null ? `[Generate a ${gender.toLowerCase()}, ${ageRange} years old${ethnicity ? `, ${ethnicity}` : ""}, clearly adult — not a minor]` : "",
    "",
  );
  ...
  lines.push(
    ...
    "",
    "Style:",
    "- modern ecommerce fashion",
    "- photorealistic",
    "- balanced lighting",
    "- clean composition",
    "- realistic fabric behavior",
    "- accurate garment rendering",
    "",
    "Avoid:",
    "- cinematic framing",
    "- dramatic lighting",
  ...
  lines.push(
    ...
    "========================================",
    "",
    "Keep outputs: stable, fast, low latency, composition consistent.",
    "Use: simple framing, moderate detail, realistic perspective, minimal scene complexity.",
    "Avoid: excessive cinematic styling, heavy editorial composition, complex camera angles, extreme perspective distortion.",
  );
...
// ────────────────────────────────────────────────────────────────────────────
// Flash master multi-angle prompt builder (back view + detail shot)
// Architecture: isolated reference images, master prompt style.
// IMAGE 1: garment reference (or back photo for back view)
// IMAGE 2: main result (identity + scene anchor)
// IMAGE 3: pose reference (if any — inserted by genAngle)
// IMAGE 4: background reference (if any)
// IMAGE 5: model photo (if any — extra identity reference, placed last)
// ────────────────────────────────────────────────────────────────────────────
function buildFlashMultiAnglePrompt(opts: {
  ...
  if (isDetail) {
    ...
    lines.push(
      ...
      "FORMAT: 3:4 portrait · FULL BODY head-to-toe.",
      "",
      "━━━ INPUT IMAGES ━━━",
      isBackWithPhoto
      ...
      "• Head turn: slight, 15–20° maximum. Front face is NOT visible.",
      "• Arms relaxed slightly away from body — back garment panel fully exposed.",
      "• Hair swept aside so back neckline of the garment is visible.",
      "",
    );
    if (isBackWithPhoto) {
      lines.push(
        ...
        `• Reproduce exactly: back neckline, back seams, rear hem, any back print or logo.`,
      ...
      lines.push(
        ...
        "• Show: back seams, rear neckline shape, back hem, fabric texture — consistent with garment front.",
    ...
    if (I_POSE !== null) {
      lines.push(
        "",
        `━━━ POSE — SKELETON FROM IMAGE ${I_POSE} ━━━`,
        `Copy back-facing body position from IMAGE ${I_POSE} — joint angles and stance ONLY.`,
        `✗ BLOCK from IMAGE ${I_POSE}: face, hair, garments, shoes, background.`,
      );
    }
    lines.push(
      "",
      "━━━ CONSISTENCY — MUST MATCH IMAGE 2 (MAIN RESULT) ━━━",
      "• Same model identity — match hair, body proportions",
      "• Same accessories, footwear, bottom wear",
      "• Same background/scene and lighting",
      "",
      "━━━ FRAMING ━━━",
      "• FULL BODY: head to shoes — all visible, no cropping.",
    );
  } else {
    lines.push(
      ...
      "",
      "━━━ VIEW SPECIFICATION ━━━",
      "• True SIDE VIEW: model rotated approximately 70°. Camera sees model's profile.",
      "• Head and feet both fully in frame. Side seam of garment clearly visible.",
      "",
      "━━━ REJECT IF ━━━",
      "✗ Model facing front or at 3/4 angle",
...
export function buildOptimizedMultiAnglePrompt(opts: {
  ...
  if (opts.angle === "side") {
    ...
    if (isBackWithPhoto) {
      viewBlock = [
        ...
        "",
        `FRAMING — CAMERA LOCK (match IMAGE ${I_MAIN} exactly):`,
        `• Use the SAME camera distance, zoom level, and body scale as IMAGE ${I_MAIN}.`,
        "• DO NOT zoom closer than the front view. DO NOT crop to torso only.",
        "• Model fills same ~80–85% of frame height. Full head-to-toe body visible.",
        "• 85mm fashion photography perspective — same focal length as front view.",
        "",
        ...
        "",
        "Avoid: front design on back output, mirrored front print, front neckline style on back, model facing forward, over-zoomed crop.",
      ];
    } else {
      ...
      viewBlock = [
        ...
        I_POSE ? `IMAGE ${I_POSE}: BACK POSE REFERENCE — skeleton/joint angles ONLY. Block: face, hair, top garment, pants/bottom garment, shoes, accessories, background.` : "",
        "",
        "Avoid: model facing forward, front design mirrored on back, hair covering back garment, over-zoomed crop.",
      ];
    }
  } else {
    // ── Detail shot — Pro prompt (angle-aware) ────────────────────────────────
    const gType = (opts.garmentType || "").toLowerCase();
    ...
    viewBlock = [
      ...
      "Requested view: DETAIL SHOT (zoomed-in garment close-up).",
      `Focus region: ${focusRegion}.`,
      `Angle: IMAGE ${I_GARMENT} (garment reference) and IMAGE ${I_MAIN} (main result) are both straight front-facing. This detail shot MUST use a DIFFERENT angle — a slight 3/4 turn or slight side angle — to provide visual variety and reveal garment depth and fabric texture.`,
      "Framing: crop tightly so the garment fills most of the frame; keep minimal background visible; exclude face and irrelevant body parts (e.g. legs for upper-body garments, head/face for lower-body garments).",
      "Garment must be centered and occupy at least 80% of the frame.",
      "Highlight fine fabric details: folds, stitching, seams, texture, and realistic shadows. Sharp focus, high resolution.",
      "Maintain the exact same garment design, color, texture, fit, and print as seen in the MAIN RESULT image. Do not alter the outfit.",
  ...
  const outputCheck: string[] = isDetail ? [
    "",
    "OUTPUT CHECK: detail shot — different angle from IMAGE 1 and IMAGE 2, garment fills 80%+ of frame, no full body, no face, garment design unchanged.",
  ] : isBackWithPhoto ? [
    "",
...
export type PrimaryGenerationOpts = {
  ...
  config: DirectCompositeConfig;
  onStep?: (step: "garment_ref" | "look_plan" | "final_prompt" | "composite") => void;
};
...
export type PrimaryGenerationResult = {
  ...
  garmentRefPrompt: string;
  compositePrompt: string;
  /** Flash only: LookPlan from Step 2 */
  lookPlan?: LookPlan;
  /** Flash only: final text prompt from Step 3 */
  finalPrompt?: string;
  timings: { garmentRefMs: number; lookPlanMs?: number; finalPromptMs?: number; compositeMs: number; totalMs: number };
};

/**
 * Primary image generation.
 *
 * Flash (gemini-2.5-flash-image): full 5-step pipeline per spec —
 *   Step 1: garment reference (cached by SHA-256)
 *   Step 2: look plan via planLookFromGarment() (text LLM)
 *   Step 3: final prompt via generateFinalPrompt() (text LLM)
 *   Step 4: composite via buildCompositePrompt() + generateImage()
 *           images: garmentRef → model? → originalGarments → pose? → bg?
 *
 * Pro (gemini-3-pro-image-preview): cost-optimized path — no text LLM calls,
 *   buildDirectCompositePrompt() builds the prompt programmatically.
 *   images: garmentRef → model? → pose? → bg?
 */
export async function generatePrimaryImage(
  ...
  const garmentRefPrompt = buildGarmentReferencePrompt(opts.garmentImages.length > 1, opts.imageModel);
  const isFlash = isFlashModel(opts.imageModel);

  // ── Step 1: garment reference — shared between Flash and Pro ─────────────
  ...
  if (cached) {
    ...
    const refResult = await generateImage({
      ...
      images: opts.garmentImages,
      aspectRatio: "3:4",
      ...(isFlash ? {} : { width: 1080, height: 1440 }),
      timeoutMs: 180_000,
    });
  ...
  // ── Flash: Steps 2–4 (full pipeline with text LLM calls) ─────────────────
  if (isFlash) {
    const flashTextModel = opts.imageModel.replace(/-image$/, "");

    // Step 2: Look Plan
    opts.onStep?.("look_plan");
    let plan: LookPlan;
    let lookPlanMs = 0;
    const tPlan = performance.now();
    try {
      const planResult = await planLookFromGarment({
        model: flashTextModel,
        garmentImages: opts.garmentImages,
        availableBackgroundThemes: opts.config.backgroundTheme ? [opts.config.backgroundTheme] : [],
        availableModelEthnicities: opts.config.modelEthnicity ? [opts.config.modelEthnicity] : [],
        userOverrides: {
          occasion:             opts.config.occasion            ?? null,
          background_theme:     opts.config.backgroundTheme     ?? null,
          footwear:             opts.config.footwear            ?? null,
          model_ethnicity:      opts.config.modelEthnicity      ?? null,
          model_pose:           opts.config.modelPose           ?? null,
          model_styling_notes:  opts.config.modelStylingNotes   ?? null,
        },
      });
      plan = planResult.plan;
    } catch {
      plan = {
        occasion:            opts.config.occasion            || "casual",
        color_scheme:        "natural, balanced",
        print_style:         "as-is",
        style_keywords:      coerceStrList(opts.config.styleKeywords),
        background_theme:    opts.config.backgroundTheme     || "clean lifestyle setting",
        footwear:            opts.config.footwear            || "appropriate footwear",
        accessories:         coerceStrList(opts.config.accessories),
        negative_prompt:     "",
        model_ethnicity:     opts.config.modelEthnicity      || "",
        model_pose:          opts.config.modelPose           || "natural standing, weight shift, relaxed hands",
        model_styling_notes: opts.config.modelStylingNotes   || "natural glam, commercial ecommerce",
      };
    }
    lookPlanMs = Math.round(performance.now() - tPlan);

    // Step 3: Final Prompt
    opts.onStep?.("final_prompt");
    let finalPrompt = "";
    let finalPromptMs = 0;
    const tPrompt = performance.now();
    try {
      const promptResult = await generateFinalPrompt({
        model: flashTextModel,
        plan,
        background: null,
        chosenModel: null,
    ...
    } catch { /* empty finalPrompt is fine — buildCompositePrompt handles it gracefully */ }
    finalPromptMs = Math.round(performance.now() - tPrompt);

    // Step 4: Composite
    // Image order: garmentRef(1) → model?(2) → originalGarments(3-N) → pose?(N+1) → bg?(N+2)
    ...
    const compositePrompt = buildCompositePrompt({
      ...
      hasPoseReference,
      hasBackgroundReference,
      originalGarmentCount: opts.garmentImages.length,
    });
    ...
    return {
      ...
      lookPlan: plan,
      finalPrompt,
      timings: { garmentRefMs, lookPlanMs, finalPromptMs, compositeMs, totalMs: Math.round(performance.now() - t0) },
    };
  }

  // ── Pro: cost-optimized composite (no text LLM calls) ────────────────────
  // Image order: garmentRef(1) → model?(2) → pose?(3) → bg?(4)
  opts.onStep?.("composite");
...
export type MultiAngleGenerationOpts = {
   ...
   * Pro: used as IMAGE 1 for back view instead of the front garment ref.
   * Flash: included in garmentImages array as additional design reference.
   */
  ...
  baseUrl?: string;
  /**
   * All original garment photos (front + back + any angles uploaded by user).
   * Flash: passed as IMAGES 2–N (secondary design reference) between garmentRef and mainResult.
   * Pro: not used (excluded for token savings).
   */
  garmentImages?: GeminiInlineImage[];
};
...
/**
 * Multi-angle image generation (back view + detail shot).
 *
 * Flash (gemini-2.5-flash-image): document-spec approach.
 *   Image order per angle: garmentRef(1) → originalGarments(2-N) → mainResult(N+1) → model?(N+2) → bg?(N+3)
 *   Prompt: buildMultiAnglePrompt() with LookPlan derived from config (no extra LLM call).
 *   Errors: caught internally — each angle returns null on failure.
 *   Execution: parallel via Promise.all.
 *
 * Pro (gemini-3-pro-image-preview): view-isolated approach.
 *   Back view uses garmentBackRawImage as IMAGE 1 (when provided).
 *   3-attempt progressive fallback for back view.
 *   Execution: sequential to stay within rate limits.
 */
export async function generateMultiAngleImages(
  ...
  // ── Flash: LookPlan from config (no extra LLM call) ──────────────────────
  const flashLookPlan: LookPlan | null = isFlash ? {
    occasion:            opts.config.occasion            || "casual",
    color_scheme:        "natural, balanced",
    print_style:         "as-is",
    style_keywords:      coerceStrList(opts.config.styleKeywords),
    ...
    negative_prompt:     "",
    model_ethnicity:     opts.config.modelEthnicity      || "",
    model_pose:          opts.config.modelPose           || "natural standing",
    model_styling_notes: opts.config.modelStylingNotes   || "natural glam, commercial ecommerce",
  } : null;

  // ── Pro refs ──────────────────────────────────────────────────────────────
  const frontRefs: GeminiInlineImage[] = [
    opts.garmentRefImage,
    opts.mainResultImage,
    ...(opts.modelImage      ? [opts.modelImage]      : []),
    ...(opts.backgroundImage ? [opts.backgroundImage] : []),
  ];
  ...
  async function genAngle(
    ...
    // ── Flash path: document-spec image order + buildMultiAnglePrompt ────────
    if (isFlash) {
      const t = performance.now();
      const garmentImages = opts.garmentImages ?? [];
      // Image order: garmentRef(1) → originalGarments(2-N) → mainResult(N+1) → model?(N+2) → bg?(N+3)
      const flashImages: GeminiInlineImage[] = [
        opts.garmentRefImage,
        ...garmentImages,
        opts.mainResultImage,
        ...(opts.modelImage      ? [opts.modelImage]      : []),
      ...
      const flashPrompt = buildMultiAnglePrompt({
        angle,
        plan: flashLookPlan!,
        finalPrompt: "",
        garmentAngleCount: garmentImages.length,
        hasModelReference,
        hasBackgroundReference,
    ...
    // ── Pro path: view-isolated (exact committed behavior) ────────────────────
    try {
      const t = performance.now();
      const isBack = angle === "back";
      const refImages = isBack ? backRefs : frontRefs;

      const images: GeminiInlineImage[] = [...refImages, ...(poseRef ? [poseRef] : [])];
  ...
  // Fetch back pose for both Flash and Pro (Pro uses it; Flash genAngle ignores poseRef)
