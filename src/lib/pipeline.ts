...
export function buildDirectCompositePrompt(opts: {
  ...
  if (opts.hasModelReference) {
    lines.push(
      ...
      "• Reproduce face structure, jawline, eyes, nose, lips, skin tone, hair color, hair texture, and all physical features EXACTLY as photographed.",
      "• Preserve the person's actual age exactly as photographed. Do NOT make the person younger, older, slimmer, or different-looking in any way.",
...
export type PrimaryGenerationOpts = {
  ...
  /** Quality tier overrides — applied to the final composite output (not garment reference). */
  width?: number;
  height?: number;
...
export async function generatePrimaryImage(
  ...
  const outWidth  = opts.width  ?? 1080;
  const outHeight = opts.height ?? 1440;
  const outTemp   = opts.temperatureOverride ?? (isFlashModel(opts.imageModel) ? 0.1 : undefined);
...
export async function generateMultiAngleImages(
  ...
  async function genAngle(
    ...
    try {
      ...
      const angleWidth  = opts.width  ?? 1080;
      const angleHeight = opts.height ?? 1440;
      const angleTemp   = opts.temperatureOverride ?? (isFlashModel(effectiveModel) ? 0.1 : undefined);
      const anglePrompt = opts.qualityBoost ? `${prompt}\n${opts.qualityBoost}` : prompt;
      ...
      const res = await generateImage({
        ...
        images,
        aspectRatio: "3:4",
        width: angleWidth, height: angleHeight,
        timeoutMs: 180_000,
        temperature: angleTemp,
      });
      ...
      console.error(`[genAngle] ${angle} view generation failed:`, err);
      return null;
    }
  }
  ...
  // Sequential to avoid concurrent Gemini API calls hitting rate limits
  const back = await genAngle("back", backPose);
  const detail = await genAngle("detail", null);
