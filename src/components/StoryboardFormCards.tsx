...
import GenerationQualityCard from "./GenerationQualityCard";
import type { GenerationTierId } from "../lib/generationTiers";
...
// ── GPT Image 2 size + quality selector ──────────────────────────────────────

import { getGptCreditKey, CREDIT_PRICES } from "../lib/pricing";
import { useCredits } from "../context/CreditsContext";

const GPT_SIZES: { value: "1024x768" | "1024x1024"; label: string; sub: string }[] = [
  { value: "1024x768",  label: "1024 × 768",  sub: "Landscape" },
  { value: "1024x1024", label: "1024 × 1024", sub: "Square" },
];

// Low quality excluded — medium and high only
const GPT_QUALITIES: { value: "medium" | "high"; label: string }[] = [
  { value: "medium", label: "Medium" },
  { value: "high",   label: "High" },
];

function GptImageSettings({
  size, quality, onSizeChange, onQualityChange,
}: {
  size: "1024x768" | "1024x1024";
  quality: "medium" | "high";
  onSizeChange: (s: "1024x768" | "1024x1024") => void;
  onQualityChange: (q: "medium" | "high") => void;
}) {
  const { modelPricing } = useCredits();
  const key = getGptCreditKey(quality, size);
  const creditCost = (modelPricing[key] ?? CREDIT_PRICES[key]) ?? 9;
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <div style={{ fontSize: 10, color: "#4b5563", marginBottom: 5, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" as const }}>
          Size
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: active ? "1.5px solid rgba(251,191,36,0.5)" : "1.5px solid rgba(255,255,255,0.07)",
                  background: active ? "rgba(251,191,36,0.08)" : "rgba(255,255,255,0.02)",
                  display: "flex", flexDirection: "column" as const, alignItems: "center", gap: 2,
                  transition: "all 0.15s",
                }}
              >
                <span style={{ fontSize: 11, fontWeight: 700, color: active ? "#fbbf24" : "#9ca3af" }}>
                  {s.label}
                </span>
                <span style={{ fontSize: 9, color: active ? "#d97706" : "#4b5563" }}>
                  {s.sub}
                </span>
              </button>
            );
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11, color: "#9ca3af" }}>
          {size} · {quality.charAt(0).toUpperCase() + quality.slice(1)} quality
        </span>
        <span style={{ fontSize: 14, fontWeight: 800, color: "#fbbf24" }}>
          {creditCost}<span style={{ fontSize: 10, fontWeight: 400, color: "#6b7280", marginLeft: 3 }}>credits/img</span>
        </span>
      </div>
    </div>
  );
          ...
          <div className="parameterSection">
            ...
            >
              ...
              <svg
                ...
                strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                style={{ transform: showAdvancedSettings ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}
              >
                <polyline points="6 9 12 15 18 9" />
              </svg>
            </button>
              ...
              <div style={{
                ...
                {config.imageModel === "gpt-image-2" && (
                  <GptImageSettings
                    ...
                    quality={(config.gptImageQuality === "high" ? "high" : "medium")}
                    onSizeChange={(s) => onConfigUpdate({ gptImageSize: s })}
                    onQualityChange={(q) => onConfigUpdate({ gptImageQuality: q })}
                  />
                )}
              </div>
            )}
